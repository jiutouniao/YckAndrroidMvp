package com.ys.jsst.pmis.commommodule.widget.refresh;
...
//				Log.d("touch", 0+"  dispatchTouchEvent  ACTION_DOWN");
//
//				downY = ev.getY();
//				lastY = downY;
//				if (mTask != null) {
...
//				Log.d("touch",0+"  dispatchTouchEvent  ACTION_MOVE  "+flag);
//				if(flag==2){ //子控件消费
//					moveDeltaY = 0;
//				}else{//下拉消费
//					handleMove(ev);
...
//				Log.d("touch",0+"  dispatchTouchEvent  ACTION_UP");
//				Log.d("touch",moveDeltaY+"ACTION_UP"+refreshDist);
//
...
//	}
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		switch (ev.getAction()){
			case MotionEvent.ACTION_DOWN:
				Log.d("touch", 1+"                   onInterceptTouchEvent  ACTION_DOWN");
				break;
			case MotionEvent.ACTION_MOVE:
				Log.d("touch",1+"                    onInterceptTouchEvent ACTION_MOVE");
				break;
			case MotionEvent.ACTION_UP:
				Log.d("touch",1+"                    onInterceptTouchEvent ACTION_UP");
				break;
		}
		Log.d("touch","flag  "+interceptType);
		if(interceptType==4){
			//拦截，自己消费
			return true;
		}
		else if(interceptType==2 ||  interceptType==3){
			//不拦截，子控件消费
			return false;
		}else{
			//默认情况不拦截
	...
	/**
	 * 0 默认情况   1.点击  2.长按之后左右上下滑动      3.左右滑动 4.上下滑动
	 */
	private int interceptType;

	/**
	 * 非长按的情况下点击才是自己消费， 其他情况都是子控件消费
	 * @param ev
	 * @return
	 */
	@Override
	public boolean dispatchTouchEvent(MotionEvent ev)
	{
		switch (ev.getActionMasked())
		{
			case MotionEvent.ACTION_DOWN:
				...
				break;
			case MotionEvent.ACTION_MOVE:
				Log.d("touch",0+"  dispatchTouchEvent  ACTION_MOVE  ");
				if(!isMove){
					final int slop = ViewConfiguration.get(getContext()).getScaledTouchSlop();
					isMove = Math.pow(Math.abs(startX- ev.getX())+Math.abs(startY- ev.getY()),2) > slop*slop;
				}
				if(!isMove){
					//点击和长按执行
					interceptType = 1;
				}else{
					if (myLongClickRunnable != null) {
						removeCallbacks(myLongClickRunnable);
					}
					if (myClickRunnable != null) {
						removeCallbacks(myClickRunnable);
					}
					Log.d("touch"," interceptType : "+interceptType  +"   "+myLongClickPerformed);
					if(interceptType<2){
...
//								super.dispatchTouchEvent(ev);
	...
	private class LongClickRunnable implements Runnable {
		@Override
		public void run() {
			Log.d("touch","myLongClickPerformed");
...
//            }
	...
	private void postLongClickRunnable() {
		myLongClickPerformed = false;
		if (myLongClickRunnable == null) {
			myLongClickRunnable = new LongClickRunnable();
		}
		postDelayed(myLongClickRunnable, ViewConfiguration.getLongPressTimeout());
	}

	/**
	 * 短按处理
	 */
	private boolean myClickPerformed;

	private ClickRunnable myClickRunnable;

	private class ClickRunnable implements Runnable {
		@Override
		public void run() {
			myClickPerformed = true;
		}
	}
	...
	private void postClickRunnable() {
		myClickPerformed = false;
		if (myClickRunnable == null) {
			myClickRunnable = new ClickRunnable();
		}
		postDelayed(myClickRunnable,0);
	}
	...
	protected void dispatchDraw(Canvas canvas)
	{
		...
		// 阴影的高度为26
		LinearGradient linearGradient = new LinearGradient(0, moveDeltaY, 0, moveDeltaY - 26, 0x009b9b9b, 0x009b9b9b, TileMode.CLAMP);
